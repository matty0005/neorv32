...
__neorv32_rom_size = DEFINED(__neorv32_rom_size) ? __neorv32_rom_size : 16K; <1>
...
----
<1> Default (max) size of the instruction memory address space (right-most value) (internal/external IMEM): 16kB
...
[start=2]
. If you have changed the default DMEM (_MEM_INT_DMEM_SIZE_ generic)
size then change the right-most parameter (here: `8kB`) so it is equal to your DMEM hardware configuration. The same goes for `neorv32_rom_size` if you have changed the default IMEM size.
